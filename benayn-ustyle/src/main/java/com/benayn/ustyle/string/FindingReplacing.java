...
public abstract class FindingReplacing<S> extends AbstrStrMatcher<S> {
	...
	protected String findingReplacing(String rRepmnt, char rMode, char rPosVar, Integer rPos) {
        ...
        Gather.from(coordinates).loop(replaceDecision);
        reset();
    ...
    class FindingReplacingDecision extends Decisional<Object[]> {
        ...
        Character rMode, rPosVar;
        String    rStr, result, intlResult, rRepmnt;

        public FindingReplacingDecision(String rResult, String rRepmnt, char rMode, Integer rPos, char rPosVar) {
            this.result = rResult;
            this.intlResult = rResult;
            this.rRepmnt = rRepmnt;
            this.rMode = rMode;
            this.rPos = rPos;
            this.rPosVar = rPosVar;
            this.betn = Betner.of(rResult);
            this.finding = Comparer.expect(rMode, 'L');
            this.replacing = Comparer.expect(rMode, 'S', 'C');
        ...
        public String result() {
            if (isRep2Find && !isCalculated) {
                asResult(replace(intlResult, result, fillRep(result.length())));
            }
        ...
        @Override
        protected void decision(Object[] input) {
            switch ((act = asAction(input)).action) {
                case 'I':
                case 'J':
                case 'K':
                    asLook(idxer.resetMode().quietly().reset(result).between(act.idxL, act.idxR));
                    break;
                case 'Z':
                    asLook(betn.reset(result).between(act.strL, act.strR).asymmetric(act.asymmL, act.asymmR));
                    break;
                case 'U':
                    asLook(act.strL);
                    break;
                ...
                case 'S':
                    betn.reset(result).between(act.strL, act.strR);
                    break;
                case 'N':
                    betn.reset(result).betweenNext(act.strL);
                    break;
                case 'L':
                    betn.reset(result).betweenLast(act.strL);
                    break;
                case 'B':
                    betn.reset(result).before(act.strL);
                    break;
                case 'A':
                    betn.reset(result).after(act.strL);
                    break;
            }
                ...
                case 'L':
                    if (finding) {
                        if (act.isAllPos) {
                            if (isRep2Find) {
                                if (act.areSames) {
                                    asResult(act.lookfor);
                                }
                                //else { /* using the first position */ act.forwIndexing(); asResult(str(act.idxForw.get(1))); }
                                else {
                                    if (act.is('A')) {
                                        asResult(str(minForwL(), result.length()));
                                    } else if (act.is('B')) {
                                        asResult(str(0, maxForwR()));
                                    } else if (act.is('L')) {
                                        act.forwIndexing();
                                        asResult(str(act.idxForw.get(1)));
                                    } else {
                                        if (act.forwIndexing().size() <= 1) {
                                            asResult(str(act.idxForw.get(1)));
                                        } else {
                                            isCalculated = true;
                                            allAsOneNegateElse(false);
                                        }
                                    }
                                }
                            } else {
                                ...
                                for (int i = 1, ifl = act.idxForw.size(); i <= ifl; i++) {
                                    b.append(str(act.idxForw.get(i)));
                                    if (i < ifl) {
                                        b.append(separator.toString());
                                    }
                                }
                                asResult(b);
                            }
                            ...
                            if (act.areSames) {
                                ...
                                act.forwIndexing();
                                asResult(str(act.idxForw.get(act.loc(1))));
                            }
                        }
                    } else if (replacing) {
                        ...
                        if (act.isAllPos) {
                            if (act.is('A')) {
                                strBuilderResultRep(minForwL(), result.length());
                            } else if (act.is('B')) {
                                strBuilderResultRep(0, maxForwR());
                            } else if (act.is('L')) {
                                act.forwIndexing();
                                strBuilderResultRep(act.idxForw.get(1));
                            } else {
                                act.versaIndexing();
                                if (act.idxForw.size() == 1) {
                                    strBuilderResultRep(act.idxForw.get(1));
                                } else {
                                    b = new StringBuilder();
                                    for (int i = 1, ifl = act.idxVersa.size(), rl = result.length(); i <= ifl; i++) {
                                        p = act.idxVersa.get(i);
                                        if (i == 1 && p.getL() > 0) {
                                            b.append(fillRep(p.getL()));
                                        }
                                        b.append(str(p));
                                        if (null != (p2 = act.idxVersa.get(i + 1))) {
                                            b.append(fillRep(p2.getL() - p.getR()));
                                        }
                                        if (i == ifl && p.getR() < rl) {
                                            b.append(fillRep(rl - p.getR()));
                                        }
                                    }
                                    asResult(b);
                                }
                            ...
                            act.forwIndexing();
                            strBuilderResultRep(act.idxForw.get(act.loc(1)));
                        }
                    }
                    break;
                case 'M':
                    ...
                    if (finding) {
                        ...
                        if (act.isAllPos) {
                            if (allAsOne) {
                                if ('S' == act.mode4Repmnt) {
                                    if (act.is('A')) {
                                        ...
                                        p = act.idxForw.get(1);
                                        asResult(new StringBuilder(str(0, p.getL()))
                                                .append(act.filler)
                                                .append(str(p.getR(), result.length())).toString());
                                    } else {
                                        allAsOneNegateElse(true);
                                    }
                                } else {
                                    for (int i = 1, ifl = act.idxForw.size(); i <= ifl; i++) {
                                        asResult(new StringBuilder(result).replace(
                                                (p = act.idxForw.get(i)).getL(), p.getR(), fill(p)).toString());
                                    }
                                }
                            } else {
                                for (int i = 1, ifl = act.idxForw.size(); i <= ifl; i++) {
                                    b.append(new StringBuilder(result).replace(
                                            (p = act.idxForw.get(i)).getL(), p.getR(), fill(p)).toString());
                                    if (i < ifl) {
                                        b.append(separator.toString());
                                    }
                            ...
                            if (null != (p = act.idxForw.get(act.loc(1)))) {
                                b.append(str(0, p.getL())).append(fill(p)).append(str(p.getR(), result.length()));
                                asResult(b);
                            }
                        }
                    } else if (replacing) {
                        if (act.isAllPos) {
                            if (act.is('A')) {
                                strBuilderResultRep(0, minForwL());
                            } else if (act.is('B')) {
                                strBuilderResultRep(maxForwR(), result.length());
                            } else if (act.is('L')) {
                                negateResultRep(act.forwIndexing().size());
                            } else {
                                act.forwIndexing();
                                Pair<Integer, Integer> p;
                                b = new StringBuilder();
        ...
        private void negateResultRep(Integer pos) {
            ...
            if (null == (p = act.idxForw.get(pos == -1 ? act.idxForw.size() : pos))) {
                return;
            }
            
            if (act.is('J')) {
                b.append(str(0, p.getL())).append(fillRep(result.length() - p.getL()));
            } else if (act.is('K')) {
                b.append(fillRep(p.getR())).append(str(p.getR(), result.length()));
            } else {
                if (p.getL() > 0) {
                    b.append(fillRep(p.getL()));
                }
                b.append(str(p));
                if (p.getR() < result.length()) {
                    b.append(fillRep(result.length() - p.getR()));
                }
            }
        ...
        private void strBuilderResultRep(Pair<Integer, Integer> p) {
            if (null != p) {
                strBuilderResultRep(p.getL(), p.getR());
            }
        }
        ...
        private void strBuilderResultRep(int l, int r) {
            ...
            asResult(new StringBuilder(result).replace(l, r, fillRep(r - l)).toString());
        }

        private Integer minForwL() {
            int min = result.length();
            act.forwIndexing();
            for (Pair<Integer, Integer> pe : act.idxForw.values()) {
                min = Math.min(min, pe.getL());
            }
            return min;
        }
        ...
        private Integer maxForwR() {
            ...
            act.forwIndexing();
            for (Pair<Integer, Integer> pe : act.idxForw.values()) {
                max = Math.max(max, pe.getR());
            }
            return max;
        }

        private void allAsOneNegateElse(boolean isAct) {
            boolean isChar = isRep2Find && 'C' == rMode;
            act.versaIndexing();
            StringBuilder b = new StringBuilder();
            Pair<Integer, Integer> p, p2 = null;
            ...
            for (int i = 1, ifl = act.idxVersa.size(); i <= ifl; i++) {
                p = act.idxVersa.get(i);
                if (isChar) {
                    p2 = act.idxVersa.get(i + 1);
                }
                if (i == 1 && p.getL() > 0) {
                    allAsOneNegateElseFill(b, isAct, isChar, p, p2);
                }
                b.append(str(p));
                if (i < ifl || (i == ifl && p.getR() < result.length())) {
                    allAsOneNegateElseFill(b, isAct, isChar, p, p2);
                }
            }
        ...
        private void allAsOneNegateElseFill(StringBuilder b, boolean isAct, boolean isChar,
                                            Pair<Integer, Integer> p, Pair<Integer, Integer> p2) {
            if (isAct) {
                b.append(fill(p));
            } else {
                if (isChar) {
        ...
        private class FarAction {
            Object[] input;
            boolean  isIdx, isStr, isAllPos, isLastPos, areSames;
            Integer  idxL, idxR, pos, asymmL, asymmR;
            String   strL, strR, filler, lookfor;
            Character action, /** forward or reverse */
                      forw, mode4Repmnt = 'S', posVar;
            Map<Integer, Pair<Integer, Integer>> idxForw, idxVersa;

            FarAction(Object[] input) {
                int iidx = -1;
                this.input = input;
                this.action = assist(++iidx);
                this.isIdx = is('I', 'J', 'K');
                this.isStr = is('N', 'L', 'S', 'Z', 'B', 'A', 'U');
                this.areSames = this.isIdx || is('Z', 'U');
                
                if (this.isIdx) {
                    this.idxL = assist(++iidx);
                    this.idxR = assist(++iidx);
                    if (null == this.idxR) {
                        this.idxR = result.length();
                    }
                }
                
                if (this.isStr) {
                    this.strL = assist(++iidx);
                    this.strR = assist(++iidx);
                }
                
                this.posVar = assist(++iidx);
                if ('X' == this.posVar) {
                    this.pos = rPos;
                    this.posVar = rPosVar;
                }
                
                this.isAllPos = 'A' == this.posVar;
                this.isLastPos = 'L' == this.posVar;
                Object tmpo = assist(++iidx);
                if (is('Z')) {
                    String[] lr = ((String) tmpo).split(WHITE_SPACE);
                    this.asymmL = Integer.parseInt(lr[0]);
                    this.asymmR = Integer.parseInt(lr[1]);
                } else if (null != tmpo) {
                    this.pos = (Integer) tmpo;
                }
                
                if (null != (tmpo = assist(++iidx))) {
                    betn.inclusive((boolean[]) tmpo);
                }
                
                this.forw = assist(++iidx);
                this.filler = assist(++iidx);
                if (null == this.forw) {
                    this.forw = 'L';
                }
            ...
            Map<Integer, Pair<Integer, Integer>> versaIndexing() {
                if (null != this.idxVersa) {
                    return this.idxVersa;
                }
                forwIndexing();
                ...
                for (int l = this.idxForw.size(), rl = result.length(), no = 0, i = 1; i <= l; i++) {
                    ...
                    if (is('B')) {
                        ...
                        p2 = this.idxForw.get(i + 1);
                        if (i == 1 && p1.getL() > 0) {
                            posesVersa.put(++no, Pair.of(0, p1.getL()));
                        }
                        if (null != p1 && null != p2) {
                            posesVersa.put(++no, Pair.of(p1.getR(), p2.getL()));
                        }
                        if (i == l && p1.getR() < rl) {
                            posesVersa.put(++no, Pair.of(p1.getR(), rl));
                        }
                    }
                }
            ...
            Map<Integer, Pair<Integer, Integer>> forwIndexing() {
                ...
                if (this.areSames) {
                    if (!Strs.isEmpty(this.lookfor)) {
                        int theIdx = 0, count = 0, ll = this.lookfor.length();
                        while ((theIdx = result.indexOf(this.lookfor, theIdx)) != INDEX_NONE_EXISTS) {
                            posesForw.put(++count, Pair.of(theIdx, ll + theIdx));
                            theIdx += ll;
                        }
                    }
                ...
                for (int i = 1; i <= c; i++) {
                    if (null != (pair = betn.position(i, true))) {
                        posesForw.put(i, pair);
                    }
                }
            ...
            Map<Integer, Pair<Integer, Integer>> detectLastPos(Map<Integer, Pair<Integer, Integer>> m) {
                if (this.isLastPos) {
                    this.pos = m.size() < 1 ? 1 : m.size();
                }
                
                return (this.idxForw = m);
            }

            boolean is(Character... acts) {
                for (Character a : acts) {
                    if (a == action) {
                        return true;
                    }
                }
            ...
            private <T> T assist(int idx) {
                return (T) ((input.length >= (idx + 1)) ? input[idx] : null);
            }

            @Override
            public String toString() {
                return Objects2.TO_STRING.apply(this);
            }
        }

        private String replace(String target, String search, String replacement) {
            if (log.isDebugEnabled()) {
                log.debug(String.format("Replace '%s' with '%s' in '%s'", search, replacement, target));
            }
            
            if (Strs.isEmpty(search) || Strs.isEmpty(target) || !target.contains(search)) {
                return target;
            }
            ...
            for (String s : l) {
                builder.append(s).append((pos == ++idx) ? replacement : (idx == len ? EMPTY : search));
            }
        ...
        private String fill(int len) {
            switch (act.mode4Repmnt) {
                case 'S':
                    return act.filler;
                case 'C':
                    return Strings.repeat(act.filler, len);
            }
            return EMPTY;
        }
        ...
        private String fillRep(int len) {
            switch (rMode) {
                case 'S':
                    return rRepmnt;
                case 'C':
                    return Strings.repeat(rRepmnt, len);
            }
            return EMPTY;
        }
        ...
        private String fill(Pair<Integer, Integer> p) {
            if (null == p) {
                return EMPTY;
            }
            return fill(p.getR() - p.getL());
        }

        private String fillRep(Pair<Integer, Integer> p) {
            if (null == p) {
                return EMPTY;
            }
            return fillRep(p.getR() - p.getL());
        }

        private String asResult(String target) {
            return (result = target);
        }

        private String asResult(StringBuilder b) {
            if (b.length() > 0) {
                return asResult(b.toString());
            }
            return b.toString();
        }

        private String asLook(String target) {
            return (act.lookfor = target);
        }

        private FarAction asAction(Object[] target) {
            return new FarAction(target);
        }

        private String str(Pair<Integer, Integer> pair) {
            if (null == pair) {
                return EMPTY;
            }
            
            return str(pair.getL(), pair.getR());
        }

        private String str(Integer l, Integer r) {
            return idxer.resetMode().reset(result).between(l, r);
        }
    }
	...
	private S delegateQueue(Object... args) {
		...
		operands = Optional.of(('Z' == (Character) args[0]) 
		        ? l.get() : ((null == args[4]) ? l.find().all() : l.find().pos((Integer) args[4])));
	...
	@Override protected void updateHandle() { reset(); }
